...
Thread counting proceeds within a central band spanning 25 to 75 percent of each dimension of the rotated mask, so the sampled region sits comfortably inside the crop and never touches the white rotation border. Each column of the band is profiled down its rows, and every dark-to-light transition along the profile marks the crossing of one thread. The transitions are accumulated over every column of the band and divided by the band area in pixels, yielding an areal thread-crossing density expressed in transitions per pixel. Multiplication by boxDim, the physical side of the sample in pixels, recovers the thread count over the full 10-centimetre sample. The warp count is obtained by transposing the warp mask so its threads run horizontally and are crossed by the vertical profiles; the weft count is obtained by profiling the weft mask directly.

The central band is what keeps the measurement honest under rotation. Gauze is flexible, so warp and weft may deviate from 90 degrees up to a worst case of 45 degrees. A rotated square's diagonal exceeds its width, so rotating the crop pushes real threads out of the corners and leaves white padding from the rotation border. Restricting the scan to the central 25 to 75 percent of each dimension guarantees no padding is ever sampled, even at a full 45-degree misalignment, while the areal-density formulation keeps the count independent of the exact choice of band size.

### Pipeline Illustration

The following images, generated in debug mode from a representative validation sample, show the principal stages of the pipeline. Each is referenced relative to the images directory of this repository.
